...
def scan_directory(path, sl, contents, current=None):
    ...
    generate a generic, JSON safe, common object representation
    ...
    folder = path.load()
    # print(dir(folder))
    folder.ignores.extend(['sized', 'action.txt', r'\._*'])
    folder.reset()

    # data from sl is not applied to folder yet, just loaded
    ...
    for remainder in folder.contents:
        ...
        if remainder.path not in default_order:
    ...
    for cur_path in default_order:
        # print(d.name)
        dpath = Path(cur_path)
        if str(dpath.filename) not in sl:
            if (re.search(r"\ $", dpath.filename)):
                # this can cause the sort order to be lost...
                # whitespaces are stripped later
                print("Warning: trailing white spaces in path: ->%s<-" % dpath.filename)
            # print("inserting: ->%s<-" % dpath.filename)
    ...
    for list_item in sl:
        for cur_path in default_order:
            ...
            if dpath.filename == list_item:
                # create the corresponding minimal, json-friendly objects
                ...
                if dpath.type() == "Directory":
                    ...
                    d = dpath.load()
                    simple = {'name': dpath.filename,
                              'image': d.default_image(),
                              'path': dpath, 'type': 'directory'}
                elif dpath.type() == "Image":
                    simple = {'name': dpath.filename,
                              'image': dpath, 'path': dpath,
                              'type': 'image'}
                else:
                    simple = {'name': dpath.filename,
                              'image': '', 'path': dpath, 'type': 'unknown'}
...
def gaze_within(source):
    ...
    if path.type() == "Directory":
        ...
        # could load it here... loop over each object
        # however,
        # loading happens as part of loading the path in scan_directory call
