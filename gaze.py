...
def scan_directory(path, sl, contents, current=None):
    ...
    for list_item in sl:
        dpath = Path(os.path.join(str(path), list_item))
        ...
        if dpath.type() == "Directory":
            ...
            if img and img.type() == "Image":
                ...
                print(img)
                img = img.load()
                thumb = str(img.size_path("small"))
                img_path = img.path
            else:
                thumb = ''
                img_path = ''
            simple = {'name': dpath.filename, 'image': img_path, 'path': dpath, 'type': 'directory', 'thumb': thumb }
        elif dpath.type() == "Image":
            img = dpath.load()
            simple = {'name': dpath.filename, 'image': dpath, 'path': dpath, 'type': 'image', 'thumb': str(img.size_path("small"))}
        else:
            simple = {'name': dpath.filename, 'image': '', 'path': dpath, 'type': 'unknown', 'thumb': ''}
    ...
    # this is an computationally expensive approach
    # trying to simplify..
